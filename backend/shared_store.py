"""
shared_store.py
Shared prediction history — visible on ALL devices simultaneously.

Backends (in priority order):
  1. JSONBin.io  – free cloud JSON store
                   Set JSONBIN_KEY + JSONBIN_BIN_ID in Render env vars
                   Initial bin content: {"predictions": []}
  2. Local file  – fallback (single-device, resets on Render redeploy)
...
def _jb_read() -> list | None:
    try:
        r = requests.get(f"{JSONBIN_BASE}/b/{JSONBIN_BIN_ID}/latest",
        ...
        log.warning(f"JSONBin read: {e}")
        return None


def _jb_write(records: list) -> bool:
    try:
        r = requests.put(f"{JSONBIN_BASE}/b/{JSONBIN_BIN_ID}",
                         headers=_jb_headers(),
...
def add_prediction(prediction: dict, match_date: str = "") -> dict:
    ...
    record  = {
        ...
        "actual_home_yellows":   None, "actual_away_yellows":   None,
        "actual_home_reds":      None, "actual_away_reds":      None,
        # Eval
        "result_correct":    None,
        "goals_error":       None,
        "corners_error":     None,
...
def resolve_prediction(record_id: int,
                       ...
                       actual_home_corners=None, actual_away_corners=None,
                       actual_home_yellows=None, actual_away_yellows=None,
                       actual_home_reds=None,    actual_away_reds=None) -> dict:
    records = load()
    updated = {}
    for i, r in enumerate(records):
        if r.get("id") == record_id:
            ...
            r["actual_home_yellows"] = actual_home_yellows
            r["actual_away_yellows"] = actual_away_yellows
            r["actual_home_reds"]    = actual_home_reds
            r["actual_away_reds"]    = actual_away_reds

            # Total bookings = yellows + (reds × 1)
            total_home_bk = (actual_home_yellows or 0) + (actual_home_reds or 0)
            total_away_bk = (actual_away_yellows or 0) + (actual_away_reds or 0)
            r["actual_home_bookings"] = total_home_bk if actual_home_yellows is not None else None
            r["actual_away_bookings"] = total_away_bk if actual_away_yellows is not None else None
            ...
            if actual_home_yellows is not None:
                phb = float(r.get("pred_home_bookings") or 0)
                pab = float(r.get("pred_away_bookings") or 0)
                r["bookings_error"] = round((abs(phb-total_home_bk)+abs(pab-total_away_bk))/2, 2)
...
def match_already_predicted(home: str, away: str, records: list = None) -> dict | None:
    """Return existing pending prediction for this matchup, or None."""
    if records is None:
        records = load()
    home_l, away_l = home.lower().strip(), away.lower().strip()
    for r in records:
        if (r.get("status") == "pending" and
            r.get("home_team","").lower().strip() == home_l and
            r.get("away_team","").lower().strip() == away_l):
            return r
    return None
